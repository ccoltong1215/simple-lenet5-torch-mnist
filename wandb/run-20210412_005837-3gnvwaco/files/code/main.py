...
####################################################################################
    ...
    plt.plot(range(int((testloader .__len__())/100)) ,lenet5_regulizedtstloss,color='r',label='test loss'     )
    plt.plot(range(int((testloader .__len__())/100)) ,CustomMLPtstloss       ,color='m' ,label='test accuracy' )
    plt.legend(loc='upper right', bbox_to_anchor=(1.0, 1.0))
    plt.xlabel('epoch (x100)')
    plt.title('TEST Loss')
    #
    customplt=fig.add_subplot(2, 2, 4)
    plt.plot(range(int((testloader.__len__()) / 100)),lenet5tstacc          , color='r', label='test loss')
    plt.plot(range(int((testloader .__len__())/100)) ,lenet5_regulizedtstacc,color='r',label='test loss'     )
